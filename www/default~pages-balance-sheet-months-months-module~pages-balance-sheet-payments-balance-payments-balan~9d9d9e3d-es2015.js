...
UserBalanceComponent.ctorParameters = () => [];
...
let BtnBackPageComponent = class BtnBackPageComponent {
    constructor(navCtr) {
        this.navCtr = navCtr;
    }
    ngOnInit() { }
...
BtnBackPageComponent.ctorParameters = () => [
...
let CreateGameComponent = class CreateGameComponent {
    constructor() {
        this.text = 'Registrar nuevo partido';
    }
    ngOnInit() { }
};
CreateGameComponent.ctorParameters = () => [];
...
let EventListComponent = class EventListComponent {
    //isAdmin = false;
    constructor(requestService, dialog, modalController, authUser, paymentTeamService) {
...
let ShowMapComponent = class ShowMapComponent {
    constructor(request, geolocation, iab, modalController) {
        this.request = request;
    ...
    getLocation() {
        return Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
            ...
            yield this.geolocation.getCurrentPosition({ timeout: 6000 })
                .then(resp => {
    ...
    */
    ...
    comoLlegar() {
        const browser = this.iab.create(`https://www.google.com/maps/dir/?api=1&destination=${this.field.lat},${this.field.lng}`, '', {
            fullscreen: "yes"
        });
    }
    shareW() {
        let text = encodeURIComponent(`https://www.google.com/maps/dir/?api=1&destination=${this.field.lat},${this.field.lng}`);
        const browser = this.iab.create(`https://api.whatsapp.com/send?text=${text}`, '', {
            fullscreen: "yes"
        });
    }
    show(field) {
...
ShowMapComponent.ctorParameters = () => [
    { type: src_app_api_request_service__WEBPACK_IMPORTED_MODULE_4__["RequestService"] },
...
let OldNextGameComponent = class OldNextGameComponent {
    constructor(router, userAuth, dialogService, modalController, dialog, util, utilArray, request) {
        this.router = router;
    ...
    confirmDelete(game) {
        this.dialog.presentAlertConfirm('Confirmar', 'Estas seguro de eliminar este partido', e => {
            this.delete(game);
        });
    }
    delete(game) {
        this.modelGame.api_delete(game.id).subscribe(data => {
            console.log(data);
            if (data['status'] == 'success') {
                this.dialog.showToast('Partido Eliminado', null, 'success');
                if (this.onDeleteGame)
...
OldNextGameComponent.ctorParameters = () => [
...
let ListAdminsComponent = class ListAdminsComponent {
    constructor() {
        this.admins = [];
    }
    ngOnInit() { }
};
ListAdminsComponent.ctorParameters = () => [];
...
let BtnPaidComponent = class BtnPaidComponent {
    constructor() { }
    ngOnInit() {
...
BtnPaidComponent.ctorParameters = () => [];
...
let MyTollbarComponent = class MyTollbarComponent {
    constructor(navCtr) {
        this.navCtr = navCtr;
        this.backDefault = '/';
...
MyTollbarComponent.ctorParameters = () => [
    { type: _ionic_angular__WEBPACK_IMPORTED_MODULE_2__["NavController"] }
];
...
let NotificatioSelectedComponent = class NotificatioSelectedComponent {
    constructor(requesService, dialoogService) {
        ...
        if (this.object) {
            if (this.isFavorite()) {
                this.favoriteModel.api_delete(this.object.favorite.id).subscribe(response => {
                    ...
                    this.dialoogService.showToast('Notificaciones inactivas');
                ...
                this.favoriteModel.api_function('addFavorite', parms).subscribe(response => {
                    if (response['status'] == 'success') {
                        ...
                        this.dialoogService.showToast('Recibirás notificaciones de las noticias relevantes de este equipo (Nuevos partidos, Resultados, etc ...)', 'Notificaciones Activas', 'primary', 4000);
                    }
                }, error => {
                    console.error(error);
                });
...
let OfflineComponent = class OfflineComponent {
    constructor(networService) {
...
let RedListComponent = class RedListComponent {
    ...
    delete(red) {
        this.redModel.api_delete(red.id).subscribe(data => {
            console.log(data);
            if (data['status'] == 'success') {
                this.dialog.showToast('Red social Eliminada', null, 'success');
...
let TeamStatisticsComponent = class TeamStatisticsComponent {
    ...
    load() {
        if (!this.list)
            if (this.id) {
                this.modelTeam.api_loadAttribute(this.id, 'statistics').subscribe(response => {
                    if (response['status'] == 'success') {
                        this.list = response['Team']['statistics'];
                    }
                });
            }
    }
...
TeamStatisticsComponent.ctorParameters = () => [
    { type: src_app_api_request_service__WEBPACK_IMPORTED_MODULE_3__["RequestService"] },
...
let TeamListAnyGameComponent = class TeamListAnyGameComponent {
    constructor(requestService, dialogService, util) {
        this.requestService = requestService;
        this.dialogService = dialogService;
        this.util = util;
    ...
    load(evet = null) {
        return Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
            if (!this.games) {
                this.teamModel.api_loadAttribute(this.id, 'games').subscribe(response => {
                    this.games = response['Team']['games'];
                    //console.log(this);
                    this.selectNextDate();
                }, error => {
                    console.log(error);
    ...
    selectNextDate() {
        ...
        if (date) {
            setTimeout(() => {
                this.util.scrollTo(date);
            }, 1000);
        }
    }
    getNextDate() {
        let ret = null;
        let date = moment__WEBPACK_IMPORTED_MODULE_6__["now"](); // this.util.setFormatMoment(new Date() , 'YYYY-MM-DD');
...
TeamListAnyGameComponent.ctorParameters = () => [
    { type: src_app_api_request_service__WEBPACK_IMPORTED_MODULE_3__["RequestService"] },
    { type: src_app_api_util_dialog_service__WEBPACK_IMPORTED_MODULE_4__["DialogService"] },
    { type: src_app_services_util_service__WEBPACK_IMPORTED_MODULE_5__["UtilService"] }
];
...
let FormPublicationPage = class FormPublicationPage extends src_app_models_modal_form__WEBPACK_IMPORTED_MODULE_10__["ModalForm"] {
    constructor(request, dialogService, viewCtrl, photoService, navParams, authUser, pathImages, location, chatService) {
...
let RedesFormPage = class RedesFormPage extends src_app_models_modal_form__WEBPACK_IMPORTED_MODULE_6__["ModalForm"] {
    constructor(request, dialogService, viewCtrl, navParams, location, chatService) {
        super(location, viewCtrl);
        this.request = request;
        this.dialogService = dialogService;
        this.viewCtrl = viewCtrl;
...
//# sourceMappingURL=default~pages-balance-sheet-months-months-module~pages-balance-sheet-payments-balance-payments-balan~9d9d9e3d-es2015.js.map