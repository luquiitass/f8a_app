...
let ListPage = class ListPage {
    ...
    init($event = null) {
        this.teamService.api_function('allTeams').subscribe(response => {
            this.list = response['data'];
            this.oldTeams = this.list;
            if ($event) {
    ...
    filterTeams(event) {
        ...
        this.list = [...this.oldTeams];
        this.list = this.list.filter(team => team.name.toUpperCase().includes(text.toUpperCase()));
    }
    cancelFilter($event) {
        this.list = this.oldTeams;
    }
