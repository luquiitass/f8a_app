...
let ResultPage = class ResultPage {
    constructor(route, requesService, dialogService, socialSharing, util, listsHomeService, navCtrl, authUser, shareService) {
        this.route = route;
    ...
    ngOnInit() {
        this.init();
        this.resultModel.api_functionModel(this.id, 'setWinner').subscribe(response => {
    ...
    delete() {
        this.resultModel.api_delete(this.game.id).subscribe(data => {
            console.log(data);
            if (data['status'] == 'success') {
                this.dialogService.showToast('Partido Eliminado', null, 'success');
                this.listsHomeService.remove('results', this.game.id);
